...
**This work has been audited.** In April 2026, three bugs were found in the SPARC dark-matter analysis predicted by the framework ([sparc-coherence-test](https://github.com/EntropyWizardchaos/sparc-coherence-test), [Coherence-Shadow](https://github.com/EntropyWizardchaos/Coherence-Shadow)). The weak claim died — luminosity-controlled p=0.026 → p=0.104. The strong claim got stronger — Q=1 subsample p=0.00006 → p=0.000002, and the morphological concentration result survived all reasonable controls (mass, gas fraction, sample restriction) at p=0.012. Real signals strengthen under audit; spurious ones die. Both outcomes are in the commit history.

**The work is runnable.** `python developmental/bt11_demo.py` runs an agent through eight developmental stages — trauma, mentor distillation, graceful aging — in two seconds on a laptop. [`foundations/Comparison.md`](foundations/Comparison.md) places the framework next to RLHF, Constitutional AI, Debate, Scalable Oversight, and Process-Based Supervision: where each is stronger, where the developmental approach adds something different, where the framework is honestly weaker.
